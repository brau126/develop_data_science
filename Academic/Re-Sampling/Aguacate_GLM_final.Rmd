...
# 5. Metodologia
La metodología empleada en este estudio se basa en un enfoque cuantitativo y un diseño de investigación descriptivo, debido al uso de variables observables, datos numéricos e información estadística con el propósito de realizar un análisis detallado sobre el cultivo de aguacate en México durante el periodo 2003–2022.

Para el diseño del seguro, se calculó la prima pura de riesgo mediante un Modelo Lineal Generalizado (GLM) con distribución Gamma y enlace logarítmico, dada la naturaleza continua, de cola pesada y asimétrica de la variable dependiente: las pérdidas agrícolas durante la siembra-cosecha de la producción. El análisis se sustentó en dos fuentes principales de datos: por un lado, la información de producción de aguacate proviene del Sistema de Información Agroalimentaria y Pesquera (SIAP), dependiente de la Secretaría de Agricultura, Ganadería, Desarrollo Rural, Pesca y Alimentación (SAGARPA); por otro lado, los datos de violencia fueron extraídos de la base de datos de la Universidad de Uppsala, que proporciona información detallada sobre eventos armados, enfrentamientos y muertes violentas entre cárteles y civiles.

Ambas bases abarcan el periodo comprendido entre 2003 y 2022, y fueron depuradas mediante Excel y R, conservando únicamente la información relevante para el análisis. En el caso de la producción de aguacate, se consideraron variables como superficie sembrada, cosechada y siniestrada, volumen de producción (en toneladas por hectárea), precios y valor económico. Para la base de datos sobre violencia, se utilizaron las muertes totales registradas por entidad federativa y año, lo cual permitió construir un índice de criminalidad por estado. Este índice se calculó como la tasa de muertes violentas por cada 100,000 habitantes, utilizando los censos poblacionales de 2000, 2005, 2010 y 2020 obtenidos del Instituto Nacional de Estadística y Geografía, e interpolando linealmente la población para los años faltantes, con el objetivo de normalizar las tasas y permitir comparaciones interanuales.

Posteriormente, se llevó a cabo un análisis exploratorio, incluyendo una tabla de frecuencias por intervalos de pérdida de producción para visualizar la distribución empírica. A partir de dicha distribución, se supuso inicialmente que los datos seguían un patrón exponencial; sin embargo, se procedió a ajustar diferentes distribuciones teóricas: exponencial, lognormal, Gamma y Pareto II. Se compararon los resultados con base en criterios de información como el AIC, BIC y loglikelihood, obteniendo que la distribución Pareto II presentaba el mejor ajuste. No obstante, se identificó un problema de sobreajuste, ya que el modelo Pareto II tendía a ajustarse de forma excesiva a los valores extremos, perdiendo capacidad de generalización y generando inestabilidad en las predicciones. Por esta razón, se optó por continuar con la distribución Gamma, que presentó el segundo mejor valor de AIC y ofreció una estructura funcional más estable.

Una vez definido el modelo GLM, se consideró como variable de respuesta la pérdida de producción (superficie sembrada menos superficie cosechada), y como variables explicativas el índice de criminalidad, el volumen de producción, el precio por tonelada y el valor total de producción. El modelo fue validado mediante diversas pruebas estadísticas (linealidad, independencia, multicolinealidad y la homocedasticidad). 

A fin de generar un modelo de proyección, se simularon datos sintéticos para las variables predictoras utilizando generadores congruenciales lineales controlados, respetando la estructura observada de los datos históricos y sus correlaciones principales. Por último, se diseñó un escenario de simulación para evaluar la exposición del reasegurador frente a eventos extremos, aplicando la simulación de bootstrap sobre los estados con índices altos de criminalidad. Se simuló un contrato de reaseguro incorporando una retención fija y un límite máximo de cobertura, calculando el Valor en Riesgo (VaR) al percentil 99.5% y generando un intervalo de confianza mediante resampling con 2000 réplicas. Este análisis permitió evaluar la prima técnica necesaria, el ratio de siniestralidad bajo condiciones adversas y la estabilidad del contrato frente a escenarios críticos. En conjunto, el presente proyecto brinda una propuesta técnica robusta útil para el diseño de seguros agrícolas en contextos de afectaciones por el crimen organizado.
...
Al ajustar el modelo lineal generalizado (GLM) con distribución Gamm, todos los coeficientes fueron estadísticamente significativos (p < 0.05), con efectos coherentes: el volumen y el precio mostraron efectos positivos, mientras que el índice de criminalidad y el valor de producción exhibieron efectos negativos.
El análisis gráfico confirmó que los residuos presentan un patrón razonablemente aleatorio, y no se observaron curvaturas severas ni indicios de mal ajuste visual. La linealidad de los parámetros fue verificada mediante gráficos de residuos parciales, respaldando la adecuación funcional de la estructura log-lineal. Asimismo, el gráfico de predicciones vs. observaciones mostró un ajuste aceptable, con una agrupación notoria de puntos en torno a la línea de identidad.
Sin embargo, al evaluar los supuestos formales del modelo, se encontraron algunas inconsistencias que merecen atención. 
La prueba de Durbin-Watson arrojó un valor de 1.6682 (p = 0.0063), lo cual indica autocorrelación positiva entre los residuos, posiblemente reflejando dependencias no modeladas entre estados o años consecutivos. Por otro lado, la prueba de Breusch-Pagan reveló evidencia significativa de heterocedasticidad (BP = 75.06, p < 0.0001). 
Aunque este resultado puede parecer conflictivo, es importante mencionar que por la naturaleza de la distribución aplicada, se espera tener heterocedasticidad, dado que la varianza se modela como una función creciente de la media.
También se detectó multicolinealidad severa entre volumenproduccion y valorproduccion, con VIF superiores a 13. Esto puede haber distorsionado parcialmente las estimaciones, aunque la significancia de los coeficientes se mantuvo, por lo que se recomienda considerar la exclusión de uno de estos predictores en análisis futuros.
Cabe señalar que, aunque inicialmente se consideró la distribución Pareto II por su alto desempeño en AIC y ajuste visual, esta generó un sobreajuste considerable. Específicamente, el modelo Pareto II presentó una sensibilidad excesiva a valores extremos, lo que redujo su capacidad de generalización y generó inestabilidad en las predicciones fuera de muestra. Por esa razón, se optó por continuar con la distribución Gamma, que, si bien no cumplió estrictamente todos los supuestos, demostró ser más robusta.
En la fase final del análisis se implementó una simulación bajo escenarios de riesgo extremo, incorporando factores de estrés y un enfoque bootstrap restringido a estados con alta criminalidad. Se construyó un escenario de reaseguro con retención y límite, y se calculó el Valor en Riesgo (VaR) al 99.5%, junto con su intervalo de confianza. Los resultados revelaron que, bajo ciertas condiciones, la prima establecida podría resultar insuficiente, y que el contrato es vulnerable a desviaciones severas cuando se concentran pérdidas en contextos con alta violencia.

# 8. Conclusiones
Los resultados confirman que el modelo Gamma logra capturar con éxito los determinantes de la pérdida productiva en el cultivo de aguacate, integrando variables productivas y de contexto social. Si bien algunos supuestos como la independencia y la homocedasticidad no se cumplieron completamente, esto es esperable en modelos GLM y no invalida su aplicabilidad, siempre que se interpreten con precaución.
La inclusión del componente de violencia resulta sumamente significativa no sólo estadísticamente, sino conceptualmente: la criminalidad puede afectar de forma directa e indirecta las actividades agrícolas, ya sea por daños materiales, alteración del comercio o cambios en la gestión territorial del riesgo.
Finalmente, el análisis de simulación bajo escenarios adversos y el uso de bootstrap permiten entender el comportamiento del modelo en las colas de distribución, ofreciendo una perspectiva práctica para la construcción de contratos de reaseguro más sensibles al contexto territorial. Esto proporciona herramientas sólidas para la toma de decisiones en esquemas de aseguramiento agrícola, con enfoque en vulnerabilidad, sobre todo en zonas de ámbito rural.
El aguacate no solo representa un producto agrícola de alto valor comercial, sino también el sustento de miles de familias. Diseñar mecanismos de protección adecuados, fundamentados en modelos estadísticos rigurosos y sensibles al territorio, permite reducir la exposición al riesgo, incentivar la inversión productiva y fortalecer la sostenibilidad del sector frente a escenarios de incertidumbre socioeconómica. Por ello, crear estos instrumentos son clave para proteger el ingreso de millones de mexicanos y garantizar la continuidad del desarrollo agroalimentario.
